...
export function buildSyncPrompt(
  ...
  return [
    ...
    '"""',
    '',
    'Output ONLY the sections that change — NEVER the whole document — using this exact patch format:',
    '<<<REPLACE',
    '## <heading copied EXACTLY from the current doc, or a brand-new heading to add>',
    '<the full new markdown for this one section: its heading line + body>',
    '>>>',
    'To delete a section:',
    '<<<REMOVE',
    '## <existing heading>',
    '>>>',
    'Patch rules:',
    '- Target the DEEPEST heading that changed: if a "## section" has "### sub-sections", REPLACE the specific "### sub-section", not the whole parent.',
    '- For a REPLACE of an existing section, copy its heading line CHARACTER-FOR-CHARACTER so it matches.',
    '- Emit a block ONLY for sections you actually add, change, or remove. If nothing changed, output nothing at all.',
    `- Never REMOVE the spine headings (${SPINE_HEADINGS.join(' , ')}); REPLACE them if their content changes.`,
    '- Nothing outside the <<<...>>> blocks: no commentary, no code fences (```).',
  ].join('\n');
}
