...
export function buildMockupPrompt(input: { doc: string; css: string; components: string }): string {
  ...
  return [
    ...
    '- GROUP the artboards by page type. Wrap each group in a section:',
    '    <section class="mock-group"><div class="mock-group-label">Group name</div> …that group\'s artboards… </section>',
    '  A group = ONE primary screen together with its own related states (its modals, overlays, expanded',
    '  panels, sub-pages). Different primary screens go in SEPARATE groups — a divider is drawn between groups.',
    '- Inside a group, one <div class="mock-art"> per screen/state:',
